<template>
  <el-card class="box-card">
    ...
    <el-table height="450" :data="list" style="width: 100%">
      <el-tree-grid prop="cat_name" label="分类名称" width="150" treeKey="cat_id" parentKey="cat_pid" levelKey="cat_level" childKey="children">
...
export default {
  ...
  methods: {
    // 添加分类 - 发送请求
    async addCate () {
      // 如果级联选择器长度为0，则添加的是一级分类
      if (this.selectedOptions.length === 0) {
        this.form.cat_pid = 0
        this.form.cat_level = 0
      }
      // 如果级联选择器长度为1，则添加的是二级分类
      if (this.selectedOptions.length === 1) {
        this.form.cat_pid = this.selectedOptions[0]
        this.form.cat_level = 1
      }
      // 如果级联选择器长度为2，则添加的是三级分类
